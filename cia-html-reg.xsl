...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="regions_list">
        <xsl:for-each select="/cia/regions/region">
            <xsl:sort select="current()"/>
            <xsl:variable name="code" select="@con_id"/>
            <h2 style="clear: both; font-size: 1.2em;letter-spacing: 0;">[<xsl:value-of select="@con_id"/>] 
                <xsl:value-of select="current()"/></h2>
            <ul>
                <xsl:for-each select="/*/*/country">
                    <xsl:variable name="regions_list" select="@region"/>
                    <xsl:if test="($regions_list= $code)">
                ...
                <div id="wrap">
                    ...
                    <div id="content">
                        <div id="main" style="width: 860px;">
                            <h1 style="text-align: center;">Региони и континенти</h1> 
                            <xsl:call-template name="regions_list"></xsl:call-template>
                        </div>
                    </div>
                    <div id="footer">

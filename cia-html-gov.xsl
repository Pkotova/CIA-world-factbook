...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="gov_list">
        <xsl:for-each select="/cia/government_types/government_type ">
            <xsl:sort select="current()"/>
            <xsl:variable name="code" select="@gov_id"/>
            <h2 style="clear: both; font-size: 1.2em;letter-spacing: 0;">[<xsl:value-of select="@gov_id"/>] 
                <xsl:value-of select="current()"/></h2>
            <ul>
                <xsl:for-each select="/*/*/country">
                    <xsl:variable name="country_list" select="@government_type "/>
                ...
                <div id="wrap">
                    ...
                    <div id="content">
                        <div id="main" style="width: 860px;">
                            ...
                            <xsl:call-template name="gov_list"></xsl:call-template>
                        </div>
                    </div>
                    <div id="footer">
